...
interface AbstractCrudFichaInitProps { object: ObjectSupport, id: string, userId?: string }

const AbstractCrudFichaInit: FC<AbstractCrudFichaInitProps> = ({ id, object, userId }) => {
    ...
    useEffect(() => {
        (async () => {
            setLoad(true);
            const ficha = await GenerateFicha({ object, id, query:`userId=${userId}` });
            const ui = await GetUiFichaCrud({ object, id });
            const resposneObeject = await GetObjectFicha({ id, object,query:`userId=${userId}` }) as any;
...
export default AbstractCrudFichaInit;
